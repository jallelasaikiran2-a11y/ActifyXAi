// ================================================================
// ActifyXAI — claude_inject.js  v7.2
...
(function () {
  ...
  try {
    chrome.runtime.sendMessage({ type: 'GET_PROMPT', llm: 'claude' }, (res) => {
      if (chrome.runtime.lastError) return;
      if (res && res.prompt) pollAndInject(res.prompt, 'claude');
    });
  } catch (e) { }
})();
...
function pollAndInject(text, llmKey) {
  ...
  const timer = setInterval(() => {
    ...
    if (editorReady()) {
      clearInterval(timer);
      // Ask background to inject in MAIN world (bypasses isolated-world limits)
      chrome.runtime.sendMessage(
        { type: 'INJECT_INTO_PAGE', text, llm: 'claude' },
        (res) => {
